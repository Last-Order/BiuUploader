...
$(document).ready(function () {
    ...
    var generateSongListItem = function generateSongListItem(item) {
        return $('<div>').addClass('list-group-item').append($('<div>').addClass('row-content').append($('<h4>').addClass('list-group-item-heading').html(item.title + ' / <small>' + item.album + '</small>')).append($('<a>').addClass('btn').addClass('btn-primary').addClass('play').text('试听').data({
            'sid': item.sid
        })));
    };
                ...
                success: function success(data) {
                    if (data.success === true) {
                        ...
                        if (data.error_code === 2) {
                            ...
                            conflictBody.append($('<a>').addClass('btn').addClass('btn-warning').text('处理撞车'));
                            conflictBody.find('.btn').click(function () {
                                $(this).remove();
                                conflictBody.append($('<h3>').text('可能撞车的音乐列表'));
                                data.result.map(function (item) {
                                    conflictBody.append($('<div>').addClass('list-group').append(generateSongListItem(item)));
                                ...
                                conflictBody.append($('<a>').addClass('btn').addClass('btn-warning').addClass('force').text('不要拦我我要传'));
